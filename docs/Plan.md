...
### 7. セマンティック検索 CLI 実装（ベクトル検索起点）
- タスク: `search [options] <query>`。オプション: `--after/--before/--mime` に加え類似度しきい値初期値（デフォルト 0.70）と最大取得件数（デフォルト 80）。GPT でキーワード 1–5 件抽出 → 「クエリ+キーワード」を Embedding → Supabase/pgvector で類似検索（期間/MIME フィルタ込み）。件数/類似度に応じた分岐:  
  - 50 件以上または最上位類似度 < 0.75: 追加質問生成→ユーザー回答を反映し再Embedding、しきい値 0.82 で再検索。  
  - 10–49 件: 上位 20 件を保持し、類似度 0.75 未満を除外。必要ならクエリリライト→再Embedding。  
  - 2–9 件: LLM リランキングで表示。  
  - 1 件: そのまま表示。  
  - 0 件: しきい値 0.60 まで緩和＋キーワード削減で再検索。再び 0 件なら「見つかりませんでした」。  
  Drive API は検索起点では使わず、候補 `file_id` の存在/権限確認や `modifiedTime` 差分取得に限定して呼ぶ。ループ上限 `SEARCH_MAX_LOOP_COUNT`。
- DOD: 分岐ロジックのユニットテスト（0/1/5/20/60 ケース）、CLI 出力が想定フォーマットで並ぶこと、ループ上限時に「10 件以下に絞り込めませんでした」と表示すること。類似度しきい値が CLI オプション/環境変数で上書き可能であること。
