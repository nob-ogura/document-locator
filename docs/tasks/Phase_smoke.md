...
> フェーズ6後半とフェーズ7前半で各1回、CI は週1回以内の夜間ジョブで実行し、失敗時は通知する。

## 推奨実施順序
1. スモーク用フォルダと環境準備（節1）
2. 認証とターゲットフォルダ確認（節2）
3. 共通スモークチェック最小セットを実行（docs/tasks/common_smoke_checks.md の 1〜3）
4. リトライ回数と件数サマリ確認（節5）
5. Supabase 反映と後処理（節6）
6. 実行スケジュールと通知確認（節7）
...
### 7. 実行スケジュールと通知
- フェーズ6後半とフェーズ7前半で各1回の手動スモークを実施し、CI では週1回以内の夜間ジョブで失敗通知（Slack / メール）を送る設定を確認する。
- Gherkin:
```gherkin
Scenario: 定義した頻度で実行し失敗を通知する
  Given フェーズ6後半のスモーク実行結果が記録されている
  And フェーズ7前半にも同様のスモーク実行が記録されている
  When CI の夜間ジョブが週1回以内の頻度で走る
  Then 失敗時に Slack またはメール通知が送信される
  And 予定外の追加スモークは頻度ポリシーを超えない
```
