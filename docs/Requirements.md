...
*   **目的**: 本書は、提供された設計コンセプトに基づき、「生成AIとベクトル検索を活用したGoogle Drive検索システム」の最小概念実証（PoC）版に関する要件を定義し、実装および受入の基準を明確化する。
...
- 特に、組織の知識資産が集約される共有ドライブ配下フォルダ（本PoCでは `.env` の `GOOGLE_DRIVE_TARGET_FOLDER_ID` で指定する単一フォルダ）が情報活用上のボトルネックになっていた。
...
*   **Google Drive クローラー (CLI)**: `.env` に設定する `GOOGLE_DRIVE_TARGET_FOLDER_ID` 配下のファイルのみを対象とし、差分同期、テキスト抽出、AIによる要約・キーワード生成、ベクトル化、データベースへの保存（Upsert）機能を提供する。
*   **セマンティック検索プログラム (CLI)**: ユーザー権限での閲覧可能ファイル取得（`GOOGLE_DRIVE_TARGET_FOLDER_ID` 配下でフィルタ済み）、クエリのベクトル化、セキュアなベクトル検索、検索結果の表示機能。
*   **対象ファイル**: **原則としてターゲットフォルダ（共有ドライブ配下）内の**Googleドキュメント、PDFなどテキスト抽出が可能なファイル。**個人のマイドライブやターゲット外フォルダは対象外とする。**
*   **データベース**: Supabase（PostgreSQL）および `pgvector` 拡張機能を利用したインデックス格納。
*   **AIモデル**: 推論に `GPT-4o mini`、ベクトル化に `text-embedding-3-small` を使用。
*   **設定**: `.env` には Google/OAuth/Supabase/OpenAIに加え、クロール対象の `GOOGLE_DRIVE_TARGET_FOLDER_ID` を記載し、環境ごとに対象フォルダを切り替えられるようにする。
...
| **KPI-3** | **インデックス鮮度** | `.env` の `GOOGLE_DRIVE_TARGET_FOLDER_ID` で指定したフォルダ内のファイル変更が検知されてからDBに反映されるまでの時間が**平均15分以内**であること。 |
...
*   クローラーは**ターゲットフォルダ（共有ドライブ配下）**のみをクロールするため、**管理者権限を持つサービスアカウント**で実行するが、検索は**必ず個々のユーザー権限**で同じフォルダ内に限定したフィルタリングを行う。
...
| ID          | 機能名                 | 要件                                                                                                                                              | 受入基準                                                                                                               |
| :---------- | :--------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------ | :--------------------------------------------------------------------------------------------------------------------- |
| **FR-C-01** | **差分変更検出**       | `.env` の `GOOGLE_DRIVE_TARGET_FOLDER_ID` を読み込み、同フォルダが属する共有ドライブの `changes.list` + Start Page Token を利用して差分を検出する。取得したイベントは `parents` / `file` 情報でフィルタし、ターゲットフォルダ配下のみを処理する。 | テスト用ターゲットフォルダでのファイル操作（作成、更新、削除）が、次回のクローラー実行時に正しく差分として検出されること。 |
...
| **FR-C-06** | **状態保存**           | 次回の差分検出に利用するため、処理完了後にターゲットフォルダに対応する共有ドライブのStart Page TokenをDBの`crawler_state`テーブルへ永続化する。     | クローラー実行後、ターゲットフォルダのStart Page Tokenが更新されていること。                                             |
...
| **FR-S-02** | **権限フィルタリング** | **最優先処理として**、ユーザー自身の認証情報でGoogle Drive APIを呼び出し、`'${GOOGLE_DRIVE_TARGET_FOLDER_ID}' in parents` で絞った閲覧可能ファイルIDのリストのみを取得する。 | 権限テストにおいて、ターゲットフォルダ外や他者専用のファイルIDがこのリストに含まれないこと。                               |
| **FR-S-03** | **セマンティック検索** | 1. クエリをベクトル化する。<br>2. **FR-S-02で取得したIDリストでDBを絞り込んだ上で**、ベクトル近傍検索を実行する。                      | **距離**（小さいほど類似度が高い）の**昇順**で結果が返され、必要に応じて距離の閾値（例: 0.5未満）でフィルタリングされること。 |
| **FR-S-04** | **結果表示**           | 検索結果を類似度の高い順（**距離の昇順**）に一覧表示する。表示項目は「ファイル名」「要約文」「Google Driveリンク」「**距離**」とする。 | 指定された4項目がコンソールに正しく出力されること。結果がない場合はメッセージを表示すること。                                 |

---

## 7. 非機能要件 (NFR)

*   **NFR-SEC-01（情報漏洩防止）**: 権限フィルタを適用しないデータベース検索は**技術的に実装を禁止**する。
...
| カラム名           | データ型                   | NULL許可 | 説明                                           |
| :----------------- | :------------------------- | :------- | :--------------------------------------------- |
| `drive_id`         | `TEXT`                     | NO       | **主キー**。`GOOGLE_DRIVE_TARGET_FOLDER_ID` が属する共有ドライブのID |
| `start_page_token` | `TEXT`                     | YES      | 上記ドライブでターゲットフォルダをクロールする際の次回Start Page Token |
...
*   **権限テスト**: 複数ユーザー（A, B）と専用/共有ファイルを用意し、Aの検索結果にBの専用ファイルが含まれないこと、逆もまた然りであることを確認する（**KPI-1**）。必ずターゲットフォルダ内のファイルのみでテストを行う。
*   **差分同期テスト**: テスト用ターゲットフォルダでファイルの「作成」「更新」「削除」を行い、クローラー実行後にDBの状態が正しく同期されていることを確認する。
...
SELECT
  ...
  file_name,
  summary,
  embedding <-> :query_vector AS distance -- L2距離を計算し、'distance'として取得
FROM
  file_index
ORDER BY
  distance ASC -- ★重要: 距離が小さい順（=類似度が高い順）にソート
LIMIT
  :top_k;
```
> **補足**: `pgvector`の`<->`演算子はL2距離（ユークリッド距離）を返す。この値は**0に近いほど、ベクトル同士が類似している**ことを意味する。そのため、`ORDER BY distance ASC` を使用して類似度の高い順に結果を取得する。`WHERE`句による`file_id`での事前フィルタリングは、セキュリティの根幹であり、省略を許容しない。
