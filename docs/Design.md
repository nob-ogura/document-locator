...
- 処理シーケンス
  ...
  2. フル or 差分判定  
     - フル: ターゲット配下を再帰 `files.list`、全件処理。  
     - 差分: `modifiedTime > drive_sync_state.drive_modified_at` のファイルのみ列挙。  
  3. テキスト抽出  
...
- 処理フロー
  1. クエリ解析  
     - 日付/期間と MIME をパース。  
     - GPT-4o mini で 3〜5 件の検索キーワード抽出（固有名詞優先）。  
  2. 初期検索（Drive API）  
...
### 2.4 LLM プロンプト方針（概要）
- キーワード抽出: 固有名詞・プロジェクト名・拡張子を優先し 3〜5 件。出力形式は JSON 配列（例: `["Foo","Bar"]`）。  
