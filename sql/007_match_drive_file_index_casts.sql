-- Align result column types with declarations
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(ts_rank_cd(d.file_name_tsv, ts_query), 0)::DOUBLE PRECISION AS lexical,
    (0.7 * (1 - (d.embedding <=> query_embedding)))
      + (0.3 * COALESCE(ts_rank_cd(d.file_name_tsv, ts_query), 0)) AS hybrid_score
