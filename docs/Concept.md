...
*   **プロセス**:
    ...
    *(注: クローラーは組織全体のファイルをインデックス化するため、管理者権限を持つサービスアカウント等で実行することを想定。個々のユーザーの検索時には、後述の検索フローで各ユーザーの閲覧権限に基づいたフィルタリングが必ず適用される。)*
...
*   **検索フロー**:
    ...
    2.  **権限フィルタの準備**:
        *   ユーザー自身の認証情報でGoogle Drive APIを呼び出し、ユーザーが閲覧権限を持つすべてのファイルIDのリストを取得する。このリストが、後続の検索における「安全な壁」として機能する。
